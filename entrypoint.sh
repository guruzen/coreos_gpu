...
KERNEL_SRC_ARCHIVE="linux-5.10.45.tar.xz"
KERNEL_SRC_DIR="${KERNEL_SRC_DIR:-/tmp/build/0/coreos_gpu/build/usr/src/linux}"
ROOT_OS_RELEASE="${ROOT_OS_RELEASE:-/root/etc/os-release}"
#NVIDIA_DRIVER_VERSION="${NVIDIA_DRIVER_VERSION:-418.43}"
NVIDIA_DRIVER_VERSION="${NVIDIA_DRIVER_VERSION:-460.73.01}"
...
NVIDIA_INSTALL_DIR_HOST="${NVIDIA_INSTALL_DIR_HOST:-/tmp/build/0/coreos_gpu/nvidia}"
NVIDIA_INSTALL_DIR_CONTAINER="${NVIDIA_INSTALL_DIR_CONTAINER:-/tmp/build/0/coreos_gpu/nvidia}"
...
check_cached_version() {
  ...
  if [[ "${VERSION_ID}" == "${CACHE_VERSION_ID}" ]]; then
    if [[ "${NVIDIA_DRIVER_VERSION}" == \
      ...
      info "Found existing driver installation for image version  \
...
run_nvidia_installer() {
  ...
  TMPDIR=NVIDIA_INSTALL_DIR_CONTAINER/NVIDIA-Linux-x86_64-460.73.01 \
  sh nvidia-installer \
